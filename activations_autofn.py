...
__all__ = ["swish_auto", "SwishAuto", "mish_auto", "MishAuto", "proposed_auto", "ProposedAuto"]
...
class ProposedAutoFn(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x):
        ctx.save_for_backward(x)
    ...
    def backward(ctx, grad_output):
        ...
        return grad_output.mul(x * x_exp * (1 - x_tanh_exp * x_tanh_exp))
 
def proposed_auto(x, inplace=False):
    # inplace ignored
    return ProposedAutoFn.apply(x)
 
class ProposedAuto(nn.Module):
    def __init__(self, inplace: bool = False):
        super(ProposedAuto, self).__init__()
        self.inplace = inplace
